...
npm install
npm run dev
# відкрити http://localhost:5173/orientir-results/?event=<id>&day=1
```

> `base` у `vite.config.ts` = `/orientir-results/` (ім'я репо), тож і локально, і на
> Pages URL містить цей префікс. Для заліку: `?event=<id>&day=summ`.
...
```
https://vadymkolesnyk.github.io/orientir-results/?event=<id>&day=<n>
```
...
Publisher додасть `?event=…&day=…` сам при друці посилань.
